...
namespace Battle_City.Core.Entities
{
    public class Enemy : Actor
    {
        ...
        private List<Tuple<int, int>> FindPath(Field _field, int startX, int startY, int goalX, int goalY)
        {
            ...
            while (openList.Count > 0)
            {
                ...
                foreach (var neighbor in GetNeighbors(_field, current))
                {
        ...
        private List<Tuple<int, int>> GetNeighbors(Field _field, Tuple<int, int> node)
        {
            ...
            foreach (var direction in directions)
            {
                ...
                if (!IsOutOfBounds(_field) && _field[newX, newY].IsWalkable()
                    && _field[newX, newY].GetType() != typeof(Reward))
                {
        ...
        public override void Shoot(Direction? direction, Field _field)
        {
            int possibility = Game.Random.Next(0, 2);
            if (possibility == 1)
            {
                base.Shoot(direction, _field);
            } else {return;}
        }
